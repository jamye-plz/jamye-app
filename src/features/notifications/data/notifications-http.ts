...
/**
 * Shared HTTP boilerplate for the notifications feature's two API clients
 * (`notifications-api.ts`, `push-installations-api.ts`): both wrap the same
 * fetch/timeout/abort/status-mapping shape but must keep their own error
 * classes (`NotificationApiError` / `PushInstallationApiError`) so callers
 * keep catching the class they already depend on. `createError`/`isApiError`
 * let each caller supply its own error identity while this module owns only
 * the mechanical request/response plumbing.
 */
