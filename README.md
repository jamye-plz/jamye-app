...
- M11: 미디어 업로드·첨부·접근 — 형식 호환성 재빌드 후 사용자 송수신 확인. 후속 승인으로 버튼 대비·영상 카드·앱 내 native 영상 재생 추가; 새 플레이어 재빌드·사용자 확인 대기. 이후 영상 미리보기 신뢰성을 개선하고 발신 단말이 만든 JPEG 포스터(서버 미배포 시 기존 로컬 프레임 추출로 fallback)를 도입했으며, 실기기 포스터 송수신 E2E 확인은 서버 배포 후로 미룬다
